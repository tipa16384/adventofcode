...
def path_find(end_node):
    ...
    open_nodes = []
    heappush(open_nodes, (0, (0, 0)))
    closed_nodes = set()

    while open_nodes:
        weight, current_node = heappop(open_nodes)
        ...
        for node in [(current_node[0]-1, current_node[1]), (current_node[0]+1, current_node[1]), 
            ...
            heappush(open_nodes, (n_weight, node))

    print(weight, timer() - start_time)

    return weight

with open('puzzle15.dat', 'r') as f: data = [list(map(int, l.strip())) for l in f.readlines()]
